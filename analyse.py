...
Rapport Word:
  ...
  et conclusion. Les fichiers intermédiaires (PNG, CSV, XLSX) sont automatiquement supprimés.
    ...
    parser.add_argument('--sap', 
                        default='EXPORT.XLSX',
                        help='Fichier Excel d\'export SAP des dépenses (défaut: EXPORT.XLSX)')
    
    parser.add_argument('--pv',
                        default='pv.xlsx',
                        help='Fichier Excel de la Planned Value (défaut: pv.xlsx)')
    
    parser.add_argument('--va',
                        default='va.xlsx',
                        help='Fichier Excel de la Valeur Acquise (défaut: va.xlsx)')
    
    parser.add_argument('--forecast',
                        default='forecast.xlsx',
                        help='Fichier Excel des projections (défaut: forecast.xlsx)')
    
    parser.add_argument('--output',
                        default='analyse_evm.png',
                        help='Fichier de sortie du graphique (défaut: analyse_evm.png)')
    
    parser.add_argument('--tableau',
                        default='tableau_evm',
                        help='Nom de base pour les fichiers tableau CSV/XLSX (défaut: tableau_evm)')
    
    parser.add_argument('--word',
                        help='Fichier Word de rapport complet à générer (ex: rapport_evm.docx)')
...
def calculer_depenses_cumulees(df, colonne_date='Date', colonne_montant='Montant'):
    ...
    depenses_cumulees = depenses_mensuelles.cumsum()
    
    return depenses_cumulees
...
def traiter_planned_value(df_pv):
    ...
    colonnes_montant = ['Montant planifié', 'Cumul planifié', 'Montant', 'montant', 'Montant prévisionnel', 'Montant prév.', 'PV', 'Planned Value']
    ...
    if len(pv_cumulee) > 1:
        ...
        pv_temp = pd.Series(pv_cumulee_complete.values, 
        ...
        pv_cumulee = pd.Series(pv_temp.values, 
...
def calculer_earned_value(df_pv, df_va):
    ...
    for mois_cible in tous_les_mois:
        ...
        for idx, row_pv in df_pv.iterrows():
            jalon = row_pv['Jalon']
    ...
    return ev_series
def calculer_projections_automatiques(depenses_cumulees, ev_cumulee, pv_cumulee, df_pv):
    """
...
def calculer_eac_projete(ev_cumulee, df_forecast, df_pv):
    ...
    for mois in mois_projection:
        if mois <= dernier_mois_ev:
            ...
            eac_cumul = ev_finale
            for jalon, info in eac_par_jalon.items():
                if info['date'] <= mois:
                    eac_cumul += info['eac']
...
def tracer_courbe(depenses_cumulees, pv_cumulee=None, jalons=None, ev_cumulee=None, eac_projete=None, jalons_forecast=None, fichier_sortie='analyse_evm.png'):
    """
    ...
    plt.plot(dates_depenses, depenses_ke.values, 
    ...
    if pv_cumulee is not None:
        ...
        plt.plot(dates_pv, pv_ke.values, 
                marker='s', linewidth=2.5, markersize=8, 
                label='PV (Planned Value - Budget prévu)', 
                color='#3498db', linestyle='--', zorder=2)
            ...
            for jalon, info in jalons_forecast.items():
                ...
                if mois in eac_projete.index:
                    ...
                    plt.annotate(jalon,
                                ...
                                fontsize=8,
                                color='#f39c12',
                                bbox=dict(boxstyle='round,pad=0.3', facecolor='white', edgecolor='#f39c12', alpha=0.8))
        ...
        if len(mois_communs_cv) > 0:
            ...
            if len(cv_dates) > 0:
                plt.annotate(f'CV: {cv_values[-1]:.1f} k€',
                            ...
                            fontsize=9,
                            color='#e67e22',
                            bbox=dict(boxstyle='round,pad=0.4', facecolor='white', edgecolor='#e67e22', alpha=0.9))
        ...
        if len(mois_communs_sv) > 0:
            ...
            if len(sv_dates) > 0:
                plt.annotate(f'SV: {sv_values[-1]:.1f} k€',
                            ...
                            fontsize=9,
                            color='#9b59b6',
                            bbox=dict(boxstyle='round,pad=0.4', facecolor='white', edgecolor='#9b59b6', alpha=0.9))
...
def tracer_courbe_realise(depenses_cumulees, pv_cumulee=None, jalons=None, ev_cumulee=None, fichier_sortie='analyse_evm_realise.png'):
    """
...
def tracer_courbe_projections(depenses_cumulees, ev_cumulee, projections, fichier_sortie='analyse_evm_projections.png'):
    ...
    for methode, couleur in couleurs.items():
        if methode in projections and projections[methode] is not None:
            ...
            plt.plot(dates_proj, series.values,
                    marker='*', linewidth=2.5, markersize=8,
                    label=f"{labels_proj[methode]} ({eac_final:.0f} k€)",
                    color=couleur, linestyle='--', zorder=2, alpha=0.8)
            
            # Ajouter annotation sur le point final
            if len(dates_proj) > 0:
                plt.annotate(f'{eac_final:.0f} k€\n{date_fin.strftime("%m/%Y")}',
                            xy=(dates_proj[-1], series.values[-1]),
                            xytext=(10, 10),
                            textcoords='offset points',
                            ha='left',
                            fontsize=8,
                            color=couleur,
                            bbox=dict(boxstyle='round,pad=0.3', facecolor='white', edgecolor=couleur, alpha=0.8))
...
def generer_tableau_comparatif(depenses_cumulees, pv_cumulee=None, ev_cumulee=None, nom_base='tableau_evm'):
    ...
    if pv_cumulee is not None and ev_cumulee is not None:
        df_tableau['SV (Schedule Variance)'] = df_tableau.get('EV (Valeur acquise)', 0) - df_tableau.get('PV (Budget prévu)', 0)
        df_tableau['CV (Cost Variance)'] = df_tableau.get('EV (Valeur acquise)', 0) - df_tableau['AC (Dépenses réelles)']
...
def generer_rapport_word(fichier_word, df_tableau, fichier_graphique, depenses_cumulees, pv_cumulee, ev_cumulee, projections, fichier_projections):
    """
    ...
    definitions = [
        ('PV (Planned Value)', 'Budget prévu ou valeur planifiée. Représente le coût budgété du travail prévu à une date donnée.'),
        ('AC (Actual Cost)', 'Coût réel ou dépenses réelles. Représente le coût réel du travail effectué à une date donnée.'),
        ('EV (Earned Value)', 'Valeur acquise ou valeur gagnée. Représente la valeur du travail réellement accompli à une date donnée, mesurée en termes de budget.'),
        ('EAC (Estimate at Completion)', 'Estimation à terminaison. Projection du coût total du projet à son achèvement.'),
        ...
        ('CPI (Cost Performance Index)', 'Indice de performance des coûts. CPI = EV / AC. Un CPI < 1 indique un dépassement de coût.'),
        ('SPI (Schedule Performance Index)', 'Indice de performance des délais. SPI = EV / PV. Un SPI < 1 indique un retard.')
    ]
    ...
    df_filtre = df_tableau[(df_tableau['AC (Dépenses réelles)'] > 0) | 
                           (df_tableau['PV (Budget prévu)'] > 0) | 
                           (df_tableau['EV (Valeur acquise)'] > 0)].copy()
    ...
    if os.path.exists(fichier_graphique):
    ...
    if cv_actuel < 0:
        doc.add_paragraph(f'⚠ Le projet présente un dépassement de coût de {abs(cv_actuel):,.2f} € à date.', style='List Bullet')
    else:
        doc.add_paragraph(f'✓ Le projet est sous budget avec une économie de {cv_actuel:,.2f} € à date.', style='List Bullet')
    
    if sv_actuel < 0:
        doc.add_paragraph(f'⚠ Le projet présente un retard équivalent à {abs(sv_actuel):,.2f} € de travail non réalisé.', style='List Bullet')
    else:
        doc.add_paragraph(f'✓ Le projet est en avance avec {sv_actuel:,.2f} € de travail supplémentaire réalisé.', style='List Bullet')
    
    if cpi < 1:
        doc.add_paragraph(f'⚠ L\'efficacité des coûts est de {cpi*100:.1f}% (chaque euro dépensé génère {cpi:.2f} € de valeur).', style='List Bullet')
    else:
        doc.add_paragraph(f'✓ L\'efficacité des coûts est de {cpi*100:.1f}% (chaque euro dépensé génère {cpi:.2f} € de valeur).', style='List Bullet')
    ...
    if os.path.exists(fichier_projections):
    ...
    if scenarios_data:
        ...
        for label, ecart, eac_val in scenarios_ecarts:
            p = doc.add_paragraph(style='List Bullet')
    ...
    if cpi < 0.9:
        doc.add_paragraph('⚠ Le CPI est très faible, indiquant une efficacité des coûts préoccupante. Actions correctives urgentes recommandées.', style='List Bullet')
    elif cpi < 1:
        doc.add_paragraph('⚠ Le CPI est inférieur à 1, indiquant un dépassement de coût. Une surveillance étroite est nécessaire.', style='List Bullet')
    else:
        doc.add_paragraph('✓ Le CPI est supérieur à 1, indiquant une bonne efficacité des coûts.', style='List Bullet')
    
    if spi < 0.9:
        doc.add_paragraph('⚠ Le SPI est très faible, indiquant un retard significatif. Révision du planning recommandée.', style='List Bullet')
    elif spi < 1:
        doc.add_paragraph('⚠ Le SPI est inférieur à 1, indiquant un retard. Des mesures d\'accélération devraient être envisagées.', style='List Bullet')
    else:
        doc.add_paragraph('✓ Le SPI est supérieur à 1, indiquant une bonne performance sur les délais.', style='List Bullet')
    
    doc.add_paragraph()
    
    # Projections - Calculer min/max/tous les EAC disponibles
    if scenarios_data:
        eacs_disponibles = [eac_val for eac_val in [eac_optimiste, eac_realiste, eac_pessimiste, eac_forecast] if eac_val is not None]
        
        if eacs_disponibles:
            doc.add_paragraph('Projections à terminaison:')
            
            if all(eac > budget_total for eac in eacs_disponibles):
                doc.add_paragraph('⚠ Tous les scénarios prévoient un dépassement de budget. Des mesures correctives sont nécessaires.', style='List Bullet')
            elif any(eac > budget_total for eac in eacs_disponibles):
                doc.add_paragraph('⚠ Certains scénarios prévoient un dépassement de budget. Une vigilance accrue est requise.', style='List Bullet')
            else:
                doc.add_paragraph('✓ Les projections indiquent un achèvement sous budget dans tous les scénarios.', style='List Bullet')
            ...
            if ecart_relatif > 10:
                doc.add_paragraph(f'⚠ L\'écart entre scénarios est important ({ecart_relatif:.1f}% du budget), reflétant une forte incertitude.', style='List Bullet')
            elif ecart_relatif > 5:
                doc.add_paragraph(f'ℹ L\'écart entre scénarios est modéré ({ecart_relatif:.1f}% du budget).', style='List Bullet')
            else:
                doc.add_paragraph(f'✓ L\'écart entre scénarios est faible ({ecart_relatif:.1f}% du budget), indiquant une bonne prévisibilité.', style='List Bullet')
    ...
    if cpi < 1 or spi < 1 or (scenarios_data and any(s['VAC (€)'] < 0 for s in scenarios_data)):
        ...
        if cpi < 1:
            doc.add_paragraph('1. Analyser les causes du dépassement de coût et identifier les postes problématiques', style='List Number')
            doc.add_paragraph('2. Mettre en place des mesures de réduction des coûts ou réviser le scope', style='List Number')
        
        if spi < 1:
            doc.add_paragraph('3. Revoir la planification et identifier les leviers d\'accélération', style='List Number')
        ...
        if scenarios_data and any(s['VAC (€)'] < 0 for s in scenarios_data):
            doc.add_paragraph('5. Prévoir un budget de contingence pour couvrir le dépassement projeté', style='List Number')
            doc.add_paragraph('6. Communiquer proactivement avec les parties prenantes sur les risques financiers', style='List Number')
    else:
...
def main():
    ...
    if args.word:
        generer_rapport_word(args.word, df_tableau, fichier_realise, depenses_cumulees, pv_cumulee, ev_cumulee, projections, fichier_projections)
